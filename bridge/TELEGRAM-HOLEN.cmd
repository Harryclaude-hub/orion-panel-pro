...
node -e "const fs=require('fs');const q=process.env.TEMP+'\orion-geheim.json';let r;try{r=JSON.parse(fs.readFileSync(q,'utf8'))}catch(e){console.log('  Antwort unlesbar. Token falsch oder abgelaufen?');process.exit(1)}if(!Array.isArray(r)){console.log('  Supabase sagt: '+JSON.stringify(r).slice(0,160));process.exit(1)}console.log('  Hinterlegte Geheimnisse: '+r.map(x=>x.name).join(', '));console.log('');const hol=n=>{const t=r.find(x=>x.name===n);return t&&t.value?String(t.value):''};const a=hol('TELEGRAM_BOT_TOKEN'),b=hol('TELEGRAM_BOT_TOKEN_KNAPP');const istPruefwert=v=>/^[0-9a-f]{64}$/.test(v);if(istPruefwert(a)||istPruefwert(b)){console.log('  ERGEBNIS: Supabase gibt die Werte NICHT heraus.');console.log('  Was zurueckkommt, sind 64 Hexzeichen - eine SHA-256-Pruefsumme,');console.log('  also nur der Fingerabdruck des Schluessels, nicht der Schluessel.');console.log('  Am 27.08. gegengeprueft: Telegram lehnt diese Werte mit 404 ab.');console.log('');console.log('  Die echten Schluessel gibt es NUR bei @BotFather in Telegram:');console.log('    /mybots  >  Bot antippen  >  API Token');console.log('  Sie sehen so aus:  1234567890:AAE...  (mit Doppelpunkt)');console.log('  Dann ORION-STARTEN.cmd doppelklicken, es fragt einmal danach.');process.exit(1)}if(!a&&!b){console.log('  Keiner der beiden Telegram-Schluessel ist dort hinterlegt.');process.exit(1)}const f='./bridge-config.json';const c=JSON.parse(fs.readFileSync(f,'utf8'));if(a)c.telegramBotToken=a;if(b)c.telegramBotTokenKnapp=b;fs.writeFileSync(f,JSON.stringify(c,null,2));console.log('  Chancen-Bot : '+(a?'eingetragen':'fehlt'));console.log('  Knapp-Bot   : '+(b?'eingetragen':'fehlt'));"
